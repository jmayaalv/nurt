  ...
  (testing "effects interceptor handles non-vector return values"
    (let [executed-effects    (atom [])
      ...
      (try
        (bus/dispatch broker {:command {:type :test-event :payload {}}})
        (catch Exception e
          (is (= {:data    {:plain-result true}
                  :message "invalid handler resposne"}
                 {:message (.getMessage e)
                  :data    (ex-data e)}))))
...
(deftest ^:parallel effects-interceptor-edge-cases
  ...
  (testing "effects with mixed keyword and map effects"
    (let [executed-effects (atom [])
          effect-fns {:log (fn [effect _] (swap! executed-effects conj [:log effect]))
                      :audit (fn [effect _] (swap! executed-effects conj [:audit effect]))
                      :email (fn [effect _] (swap! executed-effects conj [:email effect]))}
          effects-interceptor (i/effects effect-fns)
          broker (bus/in-memory-broker [:type])]

      (bus/add-handler! broker :test [:command] [effects-interceptor]
        (fn [_] [:ok {} [:log
                         {:effect/type :audit :message "test"}
                         :email
                         {:effect/type :email :to "test@example.com"}]]))

      (bus/dispatch broker {:type :test :command {}})
      (is (= [[:log :log]
              [:audit {:effect/type :audit :message "test"}]
              [:email :email]
              [:email {:effect/type :email :to "test@example.com"}]] @executed-effects))))
