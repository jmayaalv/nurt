(ns nurt.bus.macro
  "Declarative command registration macro for Kane Bus.
...
(defmacro defcommand
  ...
  (let [[docstring binding-vec options & body]
    ...
    (let [command-type        (or (:id options) (normalize-command-type name))
      ...
      `(do
         ;; Generate defmethod for nurt.broker/command-spec if :id is provided and nurt.broker exists
         ~@(when id-spec
             (try
               (require 'nurt.broker)
               (when (find-ns 'nurt.broker)
                 `((defmethod nurt.broker/command-spec ~id-spec [~'_] ~id-spec)))
               (catch Exception e
                 ;; If nurt.broker is not available at compile time, generate runtime check
                 `((try
                     (require 'nurt.broker)
                     (when (find-ns 'nurt.broker)
                       (eval (list 'defmethod 'nurt.broker/command-spec ~id-spec '[_] ~id-spec)))
                     (catch Exception ~'e
                       ;; If nurt.broker is not available, ignore silently
                       nil))))))
